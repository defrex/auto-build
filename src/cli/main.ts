...
const HELP = [
  ...
  '                                         --blocked-by takes comma-separated ids from that source; the ticket stays queued until all are done',
  ...
  switch (command) {
    ...
    case 'ticket': {
      ...
      const usage =
        ...
        '[--blocked-by id,id] (§8.8; --blocked-by ids are comma-separated and ' +
        'belong to the configured ticket source)'
      if (sub !== 'create') {
